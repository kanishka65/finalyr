...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.link-view').forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            const protectedViews = ['dashboard', 'nutrition', 'purchases', 'settings'];
            if (protectedViews.includes(view) && !AppState.getUser()) {
                showView('login');
                return;
            }

            // Update active state in sidebar
            document.querySelectorAll('.link-view').forEach(l => l.classList.remove('active'));
            this.classList.add('active');

            showView(view);

            // Auto-initialize view data
            if (view === 'dashboard' && window.initDashboard) window.initDashboard();
            if (view === 'spending' && window.initSpending) window.initSpending();
            if (view === 'nutrition' && window.initNutrition) window.initNutrition();
            if (view === 'purchases' && window.loadPurchases) window.loadPurchases();
        });
    });
